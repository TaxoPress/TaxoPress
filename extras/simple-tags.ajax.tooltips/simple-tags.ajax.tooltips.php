...
Class ST_AjaxExtension_ToolTips {
	...
	function helperJS() {
		...
		<?php else :
			wp_enqueue_script('dimensions', $this->info['install_url'].'js/jquery.dimensions.pack.js', array('jquery'));
			wp_enqueue_script('hoverIntent', $this->info['install_url'].'js/jquery.hoverIntent.minified.js', array('jquery', 'dimensions'));
			wp_enqueue_script('clueTip', $this->info['install_url'].'js/jquery.cluetip.pack.js', array('dimensions'));		
		endif; ?>
	...
	function getPostsByTags() {
		...
		global $wpdb;
		$results = (array) $wpdb->get_results("SELECT DISTINCT p.post_title, p.comment_count, p.post_date, p.ID, COUNT(tr.object_id) AS counter
