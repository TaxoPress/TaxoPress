...
function widget_st_tag_cloud_init() {
	...
	function widget_st_tag_cloud_control( $number ) {
		...
		if ( isset($_POST['widget-stags-submit-'.$number]) ) {
			foreach ( (array) $fields as $field ) {
				$newoptions[$number][$field] = strip_tags(stripslashes($_POST['widget-stags-'.$field.'-'.$number]));
			}
		}
		...
		<div>
			...
			<label for="widget-stags-title-<?php echo $number; ?>" style="line-height:35px;display:block;">
				...
				<input type="text" id="widget-stags-title-<?php echo $number; ?>" name="widget-stags-title-<?php echo $number; ?>" value="<?php echo $title; ?>" />
			</label>
	...
	function st_tag_cloud_page() {
		...
		?>
			<div class="wrap">
				<form method="post">
					<h2><?php _e('Simple Tags : Tag Cloud Widgets', 'simpletags'); ?></h2>
	...
	function widget_st_tag_cloud_register() {
		$options = get_option('widget_stags_cloud');
		$widget_ops = array('classname' => 'widget_stags_cloud', 'description' => __( 'Your most used tags in cloud format with dynamic color and many options', 'simpletags') );
