<?php
function widget_stags_cloud_init() {
	// Test Widgets
	...
	function widget_stags_cloud($args, $number = 1) {
		extract($args);
	...
	function widget_stags_cloud_control($number) {
    ...
    <script type="text/javascript">   
		controldims['#widget_stags-<?php echo $number; ?>control']['height'] = 500;
    </script>
    <div>
  ...
  <?php
	...
	function stags_cloud_setup() {
		...
		if ( $options != $newoptions ) {
			$options = $newoptions;
			update_option('widget_stags_cloud', $options);
			widget_stags_cloud_register($options['number']);
		}
	}

	function stags_cloud_page() {
		$options = $newoptions = get_option('widget_stags_cloud');
	?>
		<div class="wrap">
			<form method="post">
	...
	function widget_stags_cloud_register() {
		$options = get_option('widget_stags_cloud');
		...
		for ($i = 1; $i <= 9; $i++) {
			$name = sprintf(__('Tag Cloud %d', 'simpletags'), $i);
			$id = "widget_stags-$i";
			wp_register_sidebar_widget($id, $name, $i <= $number ? 'widget_stags_cloud' : '', $class, $i);
			wp_register_widget_control($id, $name, $i <= $number ? 'widget_stags_cloud_control' : '', $dims, $i);
		}

		add_action('sidebar_admin_setup', 'stags_cloud_setup');
		add_action('sidebar_admin_page', 'stags_cloud_page');
	}

	// Launch Widgets
	widget_stags_cloud_register();
}
add_action('plugins_loaded', 'widget_stags_cloud_init');
?>