...
Class SimpleTags {
	...
	function generateKeywords() {
		...
		if ( $number != 0 && is_array($results) && !empty($results) && count($results) > 1 ) {
			$results = shuffle($results); // Randomize keywords
	...
	WHERE (tt.taxonomy = 'post_tag'
				...
				AND (tt.taxonomy = 'category' AND tt.term_id IN ({$cat_list}))
	*/
	...
	function relatedPosts( $user_args = '', $copyright = true ) {
		...
		if ( $results === false || $results === null ) {
			...
			$inner_cat_sql = '';
			if ($include_cat != '') {
				$include_cat_sql = " AND (ctt.taxonomy = 'category' AND ctt.term_id IN ({$include_cat})) ";
	...
	function extendedPostTags( $args = '', $copyright = true ) {
		...
		$number = (int) $number;
		if ( $number != 0 ) {
			$terms = shuffle($terms); // Randomize terms
